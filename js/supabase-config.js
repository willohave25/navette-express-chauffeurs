...
function _getToken() {
  return localStorage.getItem('navette_chauffeur_token') || null;
}

function _getDriverId() {
  try {
    const u = JSON.parse(localStorage.getItem('navette_chauffeur_user') || 'null');
...
async function getTrajetsJour(chauffeurId) {
  try {
    ...
    const json = await _apiFetch('/api/trips?driver_id=' + chauffeurId + '&date_from=' + today + '&date_to=' + today);
    ...
    const mapped = list.map(t => ({
      ...
      date: t.trip_date,
      statut: t.status,
      nom_ligne: t.line?.name || t.ligne?.name || t.line_name,
      depart: t.line?.origin || t.ligne?.origin || t.origin,
      destination_ligne: t.line?.destination || t.ligne?.destination || t.destination,
      immatriculation: t.vehicle?.plate_number || t.bus?.plate_number || t.plate_number,
...
async function getPlanningHebdo(chauffeurId, offsetWeeks = 0) {
  try {
    ...
    const json = await _apiFetch('/api/trips?driver_id=' + chauffeurId + '&date_from=' + dateFrom + '&date_to=' + dateTo);
    const list = json?.data || [];
    const mapped = list.map(t => ({
      ...t,
      date: t.trip_date,
  ...
  try {
    ...
    return {
      ...
      data: {
        ...
        immatriculation: data.vehicle?.plate_number || data.plate_number,
        type_bus: data.vehicle?.type || data.model
      }
    };
  } catch (error) {
